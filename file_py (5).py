...
try:
    ...
    train_data = None
    try:
        # The fitted model stores the original data and dates within model.model.data
        raw_endog_data = model.model.data.endog
        raw_dates_data = model.model.data.dates
        train_data = pd.Series(raw_endog_data, index=pd.to_datetime(raw_dates_data))
        st.info("Training data extracted successfully with correct dates!")
    except AttributeError:
        st.error("Could not find 'model.model.data.endog' or 'model.model.data.dates'. Model object structure is unexpected.")
        ...
        raise ValueError("Training data could not be extracted or processed from the model.")
...
print(f"Dates of data saved in model: {model_fit.model.data.dates.min().strftime('%Y-%m-%d')} to {model_fit.model.data.dates.max().strftime('%Y-%m-%d')}")
